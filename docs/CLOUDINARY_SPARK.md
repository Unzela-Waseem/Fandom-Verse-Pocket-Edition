...
- Admins can still paste an existing HTTPS media URL into the form. The in-app upload buttons appear on Android, iOS, and Web only when a valid signer URL is configured.
...
5. Test a Fan avatar and an Admin content image/video, event image, and merchandise image on mobile and Web. Confirm that the returned URL uses `https://res.cloudinary.com/dc1w5stzg/`, the relevant Firestore data updates, and the media renders in the app. Test a non-Admin upload attempt, an unsupported format, an oversized file, and cancellation. This repository does not include a live-account test or deployed media host.
